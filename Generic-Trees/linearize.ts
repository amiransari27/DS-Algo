(function () {
    ...
    function getTail(node: ITreeNode<number>): ITreeNode<number> {
    ...
    function linearize(node: ITreeNode<number>): void {
        ...
        while (node.child.length > 1) {
            let last = node.child.pop()
            let secondLast = node.child[node.child.length - 1]
            let tail: ITreeNode<number> = getTail(secondLast)
            tail.child.push(last)
        }
